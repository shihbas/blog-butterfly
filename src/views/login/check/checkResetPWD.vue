<template>
    <el-dialog
        ...
        <span v-if="CheckResult === -1">正在校验重置密码信息</span>
        <span v-else-if="CheckResult === 1">重置密码成功！去登录吧！</span>
        <span v-else-if="CheckResult === 0">重置密码失败！失败原因[{{ ResultText }}]！</span>
...
<script>
import { fetchResetPwdByEmail } from '@/api/login'
...
export default {
    ...
    created() {
        ...
        fetchResetPwdByEmail(data).then(response => {
    ...
    methods: {
        handleClose() {
            if (this.checkSuccess === -1) {
                ...
                this.$confirm('重置密码成功啦！不去登录？要回到主页么？')
                .then(_ => {
                    this.$router.push({
                        name: 'homepage'
                    })
